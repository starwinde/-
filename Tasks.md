...
**P6.5 — Supabase Outbox 동기화 완성 (현재 미완성, 베타 진입 전 필수)**: 2026-05-03 기준 5중 미완성 — (1) `ScheduleRepository._enqueue()` payload 가 `'{}'` 하드코딩 (실제 row JSON 미직렬화), (2) local int id ↔ remote uuid 매핑 (`schedules.remoteId` 컬럼 정의만, 사용 0), (3) local user_id (text) ↔ remote user_id (uuid) 변환 로직 없음, (4) 신규 컬럼 `allow_disruption` / `disruption_intensity` (T5.21, rev 22) 가 remote schema 미마이그레이션, (5) Supabase RLS `uid()=user_id` 정책 활성 — anon key 만으로 insert 거부, user JWT 세션 필요. 임시 조치: 빈 payload outbox 49행 `is_synced=1` 마킹 (2026-05-03 디바이스), `outboxSyncEngineProvider` 5분 timer 그대로 동작하나 신규 enqueue 도 빈 payload 라 즉시 skip. 본격 작업 시 `.scratch/sync-completion/issues/01..05-*.md` 분해 + ADR 0005 (sync 정책) 작성 권장.
