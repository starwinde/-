...
- [ ] T5.0 Google OAuth Client ID + Gmail/Calendar API 활성화 + FCM Server Key — 사용자 액션 (rev 9 재정의, Cloudflare/Gemini 키 제거)
...
- [ ] T5.10 Google Calendar OAuth + 일정 읽기
- [-] T5.11 Gmail API + 로컬 LLM NLP via n8n 워크플로우 (신규 `routinemon-auto-schedule` 워크플로우 — llama-server-8600-v2 연결, rev 9에서 arch 변경) — **Block A1 완료 2026-04-18 (rev 10)**: n8n 워크플로우 `routinemon-auto-schedule` (ID NrqMGblcgsOTX0f4, active) + Flutter `AutoScheduleService` + `AutoScheduleRequest/Response/Source` freezed 모델 구현. TDD: unit 9 tests(models 5 + service 4) + integration 3 tests(실 n8n/llama-server) = 12 tests passed. smoke 3/3 검증 (LLM 경로 1 + preset fallback 2). analyze 0 error/warning. Gmail fetch + OAuth 연동은 T5.0 사용자 액션 후 Block C로 이월.
- [ ] T5.12 Share intent 처리 (카톡/슬랙 메시지 → NLP)
